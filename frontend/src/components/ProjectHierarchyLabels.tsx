 ...
 * project list/tile row — shared between `ProjectListPage` and
 * `FavouritesPage` rather than duplicated per page.
 *
 * Both fields are already visibility-boundary-filtered server-side
 * (`ProjectListItem.parent_project_name`/`children` — see
 * docs/decisions.md): `parent_project_name` is only ever present when the
 * caller can view that parent, and `children` only ever lists children the
 * caller can view, with no count of any hidden ones. This component just
 * renders what it's given — no client-side filtering needed or possible.
 */
export function ProjectHierarchyLabels({ project }: { project: ProjectListItem }) {

...
def create_custom_field(
    payload: CustomFieldDefinitionCreate, project: Project = Depends(require_project_manage), db: Session = Depends(get_db)
):
    count = len(
        db.scalars(
...
def delete_custom_field(
    field_id: UUID, project: Project = Depends(require_project_manage), db: Session = Depends(get_db)
):

...
def start_oidc_login(
    slug: str,
    client_nonce: str = Query(..., min_length=16, max_length=256),
    client: str = Query("app", pattern="^(app|mcp)$"),
    ...
    `client_nonce`: generated by the caller and stored in its own
    `sessionStorage` *before* it ever navigates here — round-tripped through
    `state` and the eventual callback redirect so the landing page can prove
    ...
    `client`: `"app"` (default, the frontend's own `/login/{slug}` page) or
    `"mcp"` (`mcp-server`'s own `/login` page, added when it grew SSO
    support) — selects which of two fixed, server-configured URLs
    `oidc_callback` redirects back to. Restricted to these two literal
    values by the `pattern` validation above; never an arbitrary caller-
    supplied redirect target.
    """
    org = _get_sso_org_by_slug(db, slug)
    discovery = oidc_client.discover(org.oidc_issuer_url)
    state = create_oidc_state_token(str(org.id), client_nonce, client=client)
...
def oidc_callback(code: str, state: str, request_ip: str = Depends(get_client_ip), db: Session = Depends(get_db)):
    ...
    # Only ever "app" or "mcp" (enforced by start_oidc_login's Query
    # pattern before this signed token was ever issued) — the fallback here
    # is defense in depth for a token from before this field existed, not a
    # trust boundary. Two fixed, server-configured redirect targets; never
    # anything derived from caller input.
    if state_claims.get("client") == "mcp":
        redirect_base = settings.mcp_server_public_url.rstrip("/") + "/login/oidc/complete"
    else:
        redirect_base = settings.frontend_base_url.rstrip("/") + "/oidc-complete"
    ...
    if not meets_required_group(org, claims):
        ...
        return RedirectResponse(
            f"{redirect_base}?error=not_provisioned&message={quote(message)}&client_nonce={quote(client_nonce)}",
    ...
    return RedirectResponse(
        f"{redirect_base}#token={quote(app_token)}&client_nonce={quote(client_nonce)}",
        status_code=status.HTTP_302_FOUND,
    )

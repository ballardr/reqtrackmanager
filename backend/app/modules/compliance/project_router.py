...
from app.modules.compliance.schemas import (
    ...
    ComplianceRequirementTraceabilityLinkCreate,
    ComplianceRequirementTraceabilityLinkOut,
...
# Mirrors `routers.requirements.CAN_EDIT_ROLES` exactly (as `routers.
# actions.py` already does) — traceability links onto a core requirement
# are gated by the same permission that already gates every other edit to
# that requirement's own links, not a new compliance-specific permission
# (Phase 34's own scope note).
...
#
# V1 scope (docs/compliance-module-plan.md Phase 34): initiating a link
# from the core requirement's own `RequirementDetailPage.tsx` side is the
# primary flow and is what these three endpoints serve — a picker there
# searches this organisation's compliance standards (cascading standard ->
# version -> requirement, mirroring `RequirementMappingsModal.tsx`'s
# existing precedent, since there is no cross-org compliance-requirement
# search endpoint to type-ahead against either). Initiating from the
# compliance-requirement side (e.g. from within a Standard's own
# requirement browser) is deliberately deferred, not shipped here — no
# endpoint below is scoped to make that direction impossible, but no
# frontend picker renders it in this phase.

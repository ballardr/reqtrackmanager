...
async def test_sends_warning_email_once_when_threshold_crossed():
    ...
    with patch.object(disk_monitor, "send_email_async", new=AsyncMock()) as mock_send:
        already_warned = await disk_monitor.check_disk_usage_once(backend, already_warned=False)
        assert already_warned is True
        assert mock_send.await_count == 1

        # Usage still above threshold on the next check: no repeat email.
        already_warned = await disk_monitor.check_disk_usage_once(backend, already_warned=already_warned)
        assert already_warned is True
        assert mock_send.await_count == 1
...
async def test_no_email_below_threshold_and_rewarns_after_recovery():
    ...
    with patch.object(disk_monitor, "send_email_async", new=AsyncMock()) as mock_send:
        already_warned = await disk_monitor.check_disk_usage_once(_FakeBackend(50.0), already_warned=False)
        assert already_warned is False
        assert mock_send.await_count == 0

        # Crosses the threshold, recovers, then crosses again -> warns twice.
        already_warned = await disk_monitor.check_disk_usage_once(_FakeBackend(95.0), already_warned=already_warned)
        assert mock_send.await_count == 1
        already_warned = await disk_monitor.check_disk_usage_once(_FakeBackend(50.0), already_warned=already_warned)
        assert already_warned is False
        already_warned = await disk_monitor.check_disk_usage_once(_FakeBackend(95.0), already_warned=already_warned)

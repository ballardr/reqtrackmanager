"""Regression tests for a hardening-review finding: `routers/ws.py` never
checked `User.token_version`, so a password change or 2FA disable — this
...
(corrected) claim that this had already been fixed. See
`_user_session_still_valid` and the handshake check in `routers/ws.py`.
"""
...
def test_user_session_still_valid_reflects_a_token_version_bump(client, admin_token, org_id):
    ...
    assert _user_session_still_valid(user_id, token_version=0) is True
    ...
    assert _user_session_still_valid(user_id, token_version=0) is False

...
- **Read-only unless write mode is explicitly enabled, and narrow even then.** See [Overview → Write mode](./overview.md#write-mode). Voting, commenting, submitting a change request, recording review outcomes, archiving, and file/link management are all still out of scope regardless of write mode — a deliberately narrow set, not just left for later. Approving/deciding/completing are *possible*, unlike those, but only for a project and organisation that have each explicitly opted in to AI approval and acknowledged the accountability trade-off.

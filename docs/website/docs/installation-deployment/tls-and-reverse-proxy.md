...
Do not set `PUBLIC_API_BASE_URL=/api` (or any value containing `/api`). It's a natural-looking choice — it matches the nginx location prefix above — but it's wrong, and it produces exactly the doubled-path failure this pattern is trying to avoid. The frontend's own request paths already start with `/api/v1/...`; `PUBLIC_API_BASE_URL` is *prepended* to those paths, not merged with them. So `PUBLIC_API_BASE_URL=/api` turns a request for `/api/v1/orgs` into `/api` + `/api/v1/orgs` = `/api/api/v1/orgs`, which 404s. The nginx `/api/` prefix and the frontend's own `/api/v1/...` paths were deliberately chosen to already line up — that's precisely *why* `PUBLIC_API_BASE_URL` should carry no path at all here, just empty.
:::

This exact config was verified against a real reverse proxy — an nginx container routing to the actual running dev/eval backend and frontend containers, correctly proxying both a public API endpoint and an authenticated login request through to identical responses as calling the backend directly, including the doubled-path failure mode above (deliberately reproduced and confirmed to 404 exactly as described).

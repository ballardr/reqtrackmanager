...
- **Create a requirement**: from the **Requirements** page, or the **New requirement** button on the project's **Overview** page. Pick a component, then a category belonging to that component (these determine its ID, e.g. `SW-PERF-014`) — categories live under a single component, not the whole project, so the category list narrows to match whichever component you've picked and resets if you change it. Give it a name, and optionally reasoning, a free-text description, and fill in any project-specific custom fields your organisation has defined. Every requirement must have a **target version** — the form always pre-fills one (the project's first stage, by default) rather than letting it be left blank — and a **level**: Requirement, Recommended, or Optional.
- **Import from CSV**: click **Import CSV**, choose a file, then map its columns to the required fields (name, component, category — matched by prefix within that component, not name alone — plus optional reasoning, level, and target version). A live preview shows the first few rows under your mapping before anything is uploaded, and required fields are called out so you can tell what's missing; click **Download template** for a starter CSV pre-filled with this project's own component/category prefixes. Rows that omit a target version default to the project's first stage, same as creating one directly.
...
- **Open a requirement** to see its full detail: name, reasoning, clarification, description, custom field values, an editable form (disabled once the requirement is locked), its version history (change log — including who made each change), a discussion thread for informal comments, traceability links to other requirements, and file attachments. A **Make change request** button pre-selects this requirement on the Change Requests page.
- **Attachments**: use the file picker at the bottom of the Attachments card to upload a supporting document; click a filename to download it, or the trash icon to remove it. Once a requirement is locked, this direct upload is no longer available — a file can still be attached at that point via an approved change request (see below), or informally via a discussion comment (see **Discussion**, below).
- **Discussion**: comments support file attachments too (the paperclip icon) — attach one while writing a new comment, or add/remove attachments while editing your own existing one; there's no way to attach a file to someone else's comment, or to an already-posted comment you're not editing. This works even on a locked requirement, since a discussion comment isn't part of the requirement's governed content. You can edit your own comments (click **Edit**) — an edited comment shows "(edited)" next to its timestamp so it's clear it was changed after posting.
...
1. **Create** a change request: choose "new requirement" or "modify requirement". For "modify requirement", pick the requirement first, then tick a checkbox for each field you actually want to change — Name, Reasoning, Clarification, Description, Target version, Level, Review date, Reminder lead time, Assigned reviewer, Custom fields, Attachments — only ticked fields become editable, each pre-filled from the requirement's current value so nothing needs re-typing, and only ticked fields are proposed; anything left unticked is untouched when the change request is later approved. For "new requirement", fill in the proposed name/reasoning/description/custom fields directly — the proposed category list narrows to the chosen component, the same cascading behaviour as creating a requirement directly. Either way, give a reason for the change.
2. **Submit** it — this notifies project managers and stakeholders.
3. A manager or administrator **approves** or **rejects** it with an optional decision note. Approval applies only the ticked fields to the target requirement (creating it, if it was a "new requirement" change request) and notifies the requester. A proposed attachment is only actually attached to the requirement once the change request is approved.
...
- Cast an **advisory stakeholder vote** (approve/reject, with an optional comment). This is a visible signal for whoever makes the actual decision — it does not itself approve or reject the change request; only a manager/administrator's explicit decision does that. Click **View comments** next to the vote tally to see every comment left with a vote in a pop-up.
...
Anyone who can create a stage can also **rename** it (edit its name inline) or **delete** it. Deleting a stage requires picking another existing stage to reassign everything currently targeting it to — every requirement (and its full history, not just its current version) and every pending change request proposing that stage gets moved to the one you pick. A stage with an approved baseline can't be deleted at all — baselines are a permanent record of what was approved, and deleting the stage they document would rewrite that history — and if it's the project's only stage, there's nothing to reassign to, so deletion is blocked either way.
...
- **Components and categories** (Project Admin → Categories): components and categories form a two-level tree — each category belongs to exactly one component, and its prefix only needs to be unique within that component (two different components can each have their own category with the same prefix). The tab lists each component with its own categories nested underneath, and its own inline form for adding a new one. Both can be **renamed** inline (name and prefix — renaming never changes already-issued requirement IDs like `SW-PERF-014`, only new ones going forward). **Deleting a category** requires picking another existing category to reassign its requirements to — which can belong to a different component, in which case those requirements move to that component too. **Deleting a component** is only possible once it has no categories left under it (delete or reassign each one first); there's nothing further to reassign at that point, since every requirement under it will have already moved along with its category.
...
The bell icon in the top bar shows your recent notifications (project joins, stage transitions, change-request activity, password changes, permission grants, and more), with an unread count badge. Click a notification to mark it read and — for anything about a specific requirement, change request, or project — jump straight to it; **Mark all read** clears every unread one at once without navigating anywhere. For your full notification history — searchable, and not limited to the dropdown's recent slice — go to **Notifications** in the nav's Global section, where the same click-to-open behaviour applies. In **Preferences → Notification preferences**, choose per-notification-type whether you want it in-app, by email, or both, and whether email notifications are sent instantly or batched into a daily digest.

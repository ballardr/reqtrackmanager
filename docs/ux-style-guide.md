...
*(New 2026-08-27 — human UI/UX review: "The project user page should look similar to the org user page." See `docs/decisions.md`'s "Human UI/UX review batch" entry.)*

Any member/user list scoped to a single project (today: `ProjectAdminPage.tsx`'s "Effective members" list under the Project groups tab) should follow the same structural pattern as Org Admin's Users table (`OrgAdminPage.tsx`) — a search box narrowing the list, plus a sortable table (`SortableHeader`/`cycleSort`, the same pattern documented in "Pattern: sortable column header" below) — rather than a bare, unsearchable `<ul>`. This is the pattern the earlier "Pattern: role display" section above already implicitly assumed by discussing "Org Admin's Users table" and "a project's member list" as peers; this section makes it explicit.

**Not a literal column-for-column clone.** `EffectiveMemberOut` (`backend/app/schemas/project.py`) carries `user_id`/`display_name`/`email`/`effective_role`/`sources` (role provenance — direct vs. inherited, and via what) — it has no `last_login_at`, `is_2fa_enabled`, or `is_active`, because those are org-account attributes fetched from `OrgUserOut`, not meaningful at project scope (a project doesn't know when a user last logged in anywhere, or whether their account has 2FA). A project-scoped member table's **Source** column (rendering the same provenance data the pre-existing bare list already showed, just inside a proper table cell) is the project-scope-appropriate stand-in for the org table's status/2FA/last-login columns — don't force-fit those three by inventing project-level equivalents that don't exist in the data model.

No server-side pagination for this table: `GET /{project_id}/effective-members` already returns every member in one response (per its own docstring/decision 10 — it iterates every org member server-side to resolve provenance, so there's no cheap way to paginate it further), so search/sort here are client-side, unlike Org Admin's Users table (which is genuinely paginated and sorts via query params). A future page with a real paginated project-members endpoint should sort/search server-side instead, matching Org Admin's approach rather than this one.

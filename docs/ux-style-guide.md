...
**Not every new top-level nav-rail page is a drill-down entity — "Organisation Overview" (Phase 19) is a singleton core page instead.** There is exactly one per org, never individually created/found/opened by name, so it doesn't fit this pattern's own "many created, one opened by name" tell — it's closer to a dashboard than to Projects/Standards. It's still always-visible (general org stats apply regardless of which modules are installed, unlike a module-gated tab), so it's declared directly in `Layout.tsx`/`App.tsx` as a core route, not through a module's `globalNavItems`/`globalRoutes`. What *is* reused from this pattern's own module-extension shape: a module's own org-wide surface (e.g. Compliance's dashboard/standards/outstanding view, previously an `orgAdminSections` group) can register a `ResourceMenu` section on this page too, via the sibling `orgOverviewSections` field on `TierAModuleDefinition` — same `{key, label, render({orgId})}` shape as `orgAdminSections`, same "module decides, core just mounts" boundary, just contributing to a different core page.

**A drill-down entity's Overview should be stats-forward, not a plain description card (Phase 23).** Compliance Standards' own Overview originally rendered only description text and Edit/Archive buttons — zero counts, zero navigation shortcuts, unlike a project's own Overview. The fix, and the convention every future drill-down entity's Overview should start from: a `.grid.grid-metrics` row of `MetricTile`s at the top of the section, summarising the entity's own scale (child-collection counts, cross-entity usage) with each tile a real link into the sub-view it summarises — the same "stat tile → click navigates to a filtered view" idiom `ProjectOverviewPage.tsx`/`OrgOverviewPage.tsx` already established, not a third convention. Reuse `MetricTile` (clickable, a `<Link>`) for these, not the plain, non-clickable `StatCard` — see `MetricTile`'s own docstring for that distinction. Where the entity's Overview also needs to show a cross-entity usage slice (e.g. "how many projects use this standard, and how many are compliant"), narrow an existing cross-entity aggregation endpoint to this one entity server-side rather than inventing new counting logic — see `service.py::build_status_out`'s reuse via `GET .../standards/{id}/project-summary` for the concrete precedent.

**A drill-down entity with its own many-child collection should offer an expandable nav-rail group, not force every visit through the plain list first** — see "Pattern: expandable nav-rail group" below, added alongside this Overview-stats convention from the same review (Compliance Standards' own Versions).
...
The fix, introduced for Compliance Standards' "Versions" entry (Phase 23) and intended as the reusable shape for the next entity that needs it:

```mermaid
flowchart LR
  A["NavRailLink: Versions\n(+ disclosure toggle)"] -->|"click the collection link itself"| B["Plain list page\n/standards/:id/versions"]
  A -->|"click the disclosure toggle"| C["Expanded: one link per version"]
  C -->|"click v2.0"| D["/standards/:id/versions/:versionId\n(VersionWorkspace, opened directly)"]
  B -->|"click a row in the list"| D
```

- The nav-rail entry for the collection gains a disclosure toggle beside its own `NavRailLink` — the same `ChevronDown`/`ChevronRight` pair (collapsed → expanded) `CollapsibleSection` already establishes for the same meaning elsewhere in the app, but **not** `CollapsibleSection` itself: that component's own card chrome (its own header row, background, padding) doesn't fit inside a nav-rail row, so the toggle is a plain icon-button rendered directly beside the `NavRailLink`, sized and styled like every other nav-rail control.
- Expanding the toggle lists one small link per child item underneath, indented, each linking straight to that child's own more specific URL (`/standards/:id/versions/:versionId`, sharpening the collection's own plain `/standards/:id/versions`) — a real route, not client-only state, so the destination page can itself be reached directly (deep-linked, bookmarked, opened from a stat tile) without visiting the plain list first.
- The expanded/collapsed choice persists per-user via `useUiPreference` (`section_collapsed:<key>:<entityId>`, one stored preference per entity instance — a user who expands Standard A's Versions isn't forced to also see Standard B's expanded), the same mechanism `nav_rail_collapsed`/`section_collapsed:*` already use elsewhere.
- The toggle only renders when the collection is non-empty and the rail itself isn't in its collapsed, icons-only mode — an empty disclosure control, or one crammed into an icons-only rail with no room for indented sub-links, isn't worth building.
- The destination page that owns "which child is open" reads it from that same route param (not a locally-invented one) — see Compliance's own `StandardVersionsSection.tsx`, which moved from a locally-`useState`-held "active version" to deriving it from `StandardWorkspacePage.tsx`'s `:versionId?` param specifically so this nav-rail shortcut and the plain list's own "open a version" click land on the identical URL/state.
...
This is a display rule for summary contexts, not a data change. **Revised 2026-08-30 (reverses the 2026-08-24/25 decision below, at the user's explicit direction — see `docs/decisions.md`):** Org Admin's "View access" panel — the genuine access-audit view, specifically built to answer "what does this person actually have" — now also uses the collapsed summary as each project row's *default* display, with a small per-row "Show all N roles" toggle that reveals the full, uncollapsed role set on demand. This keeps the audit detail reachable (nothing was removed — a click away, not gone) without making it the default view a PM has to scan past for every project row. Apply the collapsing rule as the default everywhere a `ProjectRole` list is shown — compact list rows (Org Admin's Users table, a project's member list) always collapsed with no expand option, and the access-audit panel collapsed by default with an explicit expand-on-demand affordance.

**Applied** as `collapseProjectRoles()` (`frontend/src/api/types.ts`), a small precedence helper next to `PROJECT_ROLE_LABEL` rather than duplicated per call site. Wired up at the app's real compact `ProjectRole`-badge-list call sites, found by grepping every `PROJECT_ROLE_LABEL` usage rather than assuming the roadmap item's own worked example: `ProjectListPage.tsx` (both tile and list/table views), `FavouritesPage.tsx`, and `PreferencesPage.tsx`'s "my organisations" project list — all render a user's *own* `my_roles` for a project as a compact badge list, always collapsed with no expand affordance (this is a personal-summary context, not an audit one). Org Admin's "View access" `SidePanel` (`OrgAdminPage.tsx`'s `viewingUser` panel) is now collapsed-by-default too, but per-row expandable, per the 2026-08-30 revision above. Left uncollapsed and out of scope for this pattern: Org Admin's Users table `Roles` column, which — checked directly rather than assumed from the roadmap item's own "collapse project/org role badges" phrasing — renders `OrgRole` (`org_admin`/`project_creator`/`member`), a different, unordered three-value enum with no precedence defined by this pattern, not `ProjectRole`; collapsing it would need a new rule this section doesn't specify. See `docs/decisions.md` for this scope correction. That same `OrgRole` column's control shape — an always-visible checkbox stack, not a dropdown — is addressed separately below, in `Pattern: multi-select dropdown`.
...
`FilterPanel`/`FilterField`/`FilterCheckbox` (`frontend/src/components/FilterPanel.tsx`) is the right-hand narrowing-filter sidebar: a plain `.card.stack` shell (`FilterPanel`) holding one or more labelled controls (`FilterField` wraps a `<label>` around its child, giving every filter control a real accessible name for free — no separate `aria-label` needed; `FilterCheckbox` is the boolean equivalent for a plain toggle like "unread only"). By default (`layout="side"`) it sits in the same `.side-grid` layout slot: list/table content on the left, `FilterPanel` on the right, both visible at once — see "Pattern: filter panel placement — side vs. top" below for the `layout="top"` alternative used above a handful of wide, many-column tables. *(Header/body split below, 2026-08-27: on desktop this whole panel is still never collapsed behind a menu; below the mobile breakpoint the filter *body* specifically does collapse — the header never does. This applies to both layouts.)*

Reach for it when a list page needs one or more controls that *narrow which rows are shown* — a status/role/stage/component select, a reviewer or action-type picker, a boolean like "only watched" or "include archived." It is specifically for narrowing filters, not for the page's primary actions or a tile/list `ViewToggle` — those stay in the main content area above the list, matching the split every current call site already uses (see the "Consolidate filter UI onto `FilterPanel` everywhere needed" roadmap item, which moved four pages onto this exact split rather than inventing a new one). A page's free-text search box is different: see the header/body split below — it now lives in `FilterPanel`'s own header on the two pages that have one. Six pages use it today: `RequirementsPage.tsx` and `ChangeRequestsPage.tsx` (the original two, Status/Category/Target-version/Reviewer-shaped selects), `ProjectListPage.tsx` (Status/Organisation/Role/Stage-status), `ProjectReviewsDuePage.tsx` (Component/Reviewer), `ProjectActionsPage.tsx` (Action type/Outcome), `ServerOrganisationsPage.tsx` (Status), and `NotificationsPage.tsx` (a single `FilterCheckbox`, "Unread only" — no `FilterField` selects, proving the panel doesn't require multiple controls to earn its place).
...
**Header + collapsible body.** *(New 2026-08-27 — result-count/dashboard-navigation hardening pass, see `docs/decisions.md`.)* `FilterPanel` is now structurally two parts, not one flat stack of children:

- **Header** (always rendered, never collapses): a `ResultCount` (`frontend/src/components/ResultCount.tsx`) showing the list's result count — just the total when no filter/search is narrowing it (`"57 total"`), or both counts once one is (`"Showing 12 matching · 57 total"`, the filtered count first) — plus, on `RequirementsPage.tsx` and `ProjectListPage.tsx`, the page's free-text search `<input>`, relocated here from the main content column. `ChangeRequestsPage.tsx` has no search box at all (unchanged), so its header is `ResultCount` alone.
- **Body** (`children` — the existing `FilterField`/`FilterCheckbox` controls): on desktop/wide viewports renders exactly as before, fully visible with no toggle. Below the same mobile breakpoint `.side-grid` itself collapses at (`theme.css`, `@media (max-width: 860px)`), the body defaults to collapsed and is wrapped in the existing `CollapsibleSection` accordion (`Pattern: settings hierarchy`'s third leaf, above) — reused rather than inventing a second disclosure widget — with a toggle to expand it. The always-visible header means the result count and search stay reachable on a phone-width screen without needing to open the filters first.
...
`FilterPanel`'s default (`layout="side"`, unchanged) puts it in the `.side-grid` sidebar slot next to the table — right for a *narrow* directory, roughly **4 columns or fewer** (Org Groups, Project Groups: Name / role-or-none / Members). A 240px sidebar barely registers against a table that narrow.

`layout="top"` renders `FilterPanel` as a full-width horizontal bar *above* the table instead — the header (`ResultCount` + search) and the filter fields (`children`) both flow in a wrapping row rather than a vertical stack, and the panel drops the sidebar's `minWidth`/`alignSelf` so it can span the page/section width. Use it for a *wide* directory — roughly **5+ columns**, or any table whose columns include a multi-select dropdown or free-flowing provenance text that needs real room — where a 240px sidebar would otherwise crowd the table. Three call sites moved to it in this fix: Org Admin's Users table (6 columns: Email, Name, Role, Last login, 2FA, Actions), Server Admin's Access Review (similar width), and `ProjectMembersTable.tsx` (4 columns, but Role is a `MultiSelectDropdown` and Source holds multi-line provenance text — both call sites, Project Admin's Members section and Org Admin's "Manage users" modal, inherit the fix automatically since it's one shared component).

Below the mobile breakpoint, `layout="top"` collapses its filter fields behind the same `CollapsibleSection` accordion `layout="side"` already uses — only the header (result count + search) stays always-visible in both layouts; nothing about the mobile behavior differs between them.
...
## Pattern: project-scoped member lists match Org Admin's Users table

*(New 2026-08-27 — human UI/UX review: "The project user page should look similar to the org user page." See `docs/decisions.md`'s "Human UI/UX review batch" entry. Superseded in substance, not in principle, by the follow-up UX batch's Phase D, 2026-08-31 — `ProjectMembersTable` is the single implementation now shared by both surfaces this section originally asked to be *similar*.)*

Project Admin's Members section and Org Admin's "Manage users" modal both render `ProjectMembersTable` (`frontend/src/components/ProjectMembersTable.tsx`) — the same component, not two independently-styled lookalikes — which itself follows the same structural pattern as Org Admin's own Users table: a search box narrowing the list, plus a sortable `DirectoryTable` (`SortableHeader`/`cycleSort`, "Pattern: sortable column header" below). This is a stronger form of the original ask ("look similar to") — the two surfaces can't drift apart in future changes the way two separately-maintained "similar" implementations eventually would.

**Not a literal column-for-column clone of Org Admin's Users table.** `EffectiveMemberOut` (`backend/app/schemas/project.py`) carries `user_id`/`display_name`/`email`/`effective_role`/`sources` (role provenance — one of five direct kinds, or forward/member-source-inherited, and via what) — it has no `last_login_at`, `is_2fa_enabled`, or `is_active`, because those are org-account attributes fetched from `OrgUserOut`, not meaningful at project scope (a project doesn't know when a user last logged in anywhere, or whether their account has 2FA). `ProjectMembersTable`'s **Source** column is the project-scope-appropriate stand-in for the org table's status/2FA/last-login columns — don't force-fit those three by inventing project-level equivalents that don't exist in the data model. Its **Role** column also differs by row kind, deliberately: a real member gets a `MultiSelectDropdown` (checked options come from `sources`, and only a purely-`direct_role`-sourced option is togglable — see `MemberSourceProvenanceKind`'s own doc comment, `frontend/src/api/types.ts`, for why the other four kinds must render checked-but-disabled with an explanatory title instead); an invited row (`kind: "invited"`, merged in from `GET /{project_id}/pending-invites`) gets a status badge + Resend button instead.

**Client-side search/filter/pagination, not server-side, despite `GET /{project_id}/effective-members` supporting `search`/`limit`/`offset` as of Phase D.** `get_effective_project_members_with_provenance` still resolves every candidate user's full provenance in Python before any slicing (per its own docstring — it iterates every org member server-side), so the backend params are application-level pagination over an already-fully-resolved result, not real database-level pagination. `ProjectMembersTable` doesn't use them for its primary fetch anyway: role-filtering and the pending-invite merge both need the *complete* loaded set to stay correct, and a `LoadMoreButton` backed by real server-side offset pagination would make the role filter silently incomplete for any member not yet loaded. This is the same "small enough to load whole, search/filter/paginate client-side" precedent `MemberRoleTable`'s own two data sources set before it, not a regression.

## Pattern: pre-auth branding header

*(New 2026-08-27 — human UI/UX review: "The server login page should have the branding logo and branding header title above the login text." See `docs/decisions.md`'s "Human UI/UX review batch" entry.)*
...
Read top to bottom: the branch point is what kind of list this is; everything below a "Directory" answer is required together, everything below "Feed" is already the existing, correctly-applied pattern. *Why:* this used to describe two real gaps — Org Admin's Users table had three fixed audit-filter buttons but no way to search by name or email, and neither Users nor Groups paginated at all; Org Admin's and Project Admin's Groups sections both rendered every group's full member list open and inline, unconditionally, which was unnoticeable at a handful of seed-data groups and became the entire page at a hundred. Both are now fixed: Org Admin's Users table (Phase A, 2026-08-31 follow-up UX batch) got real search, `limit`/`offset` pagination, and its filters moved into `FilterPanel`; Org Admin's and Project Admin's Groups sections (Phase B, same batch) both moved onto `DirectoryTable` (see "Pattern: directory table" below) with a per-row `SidePanel` replacing the always-expanded accordion — a group's member list is now collapsed by default the way this pattern requires, since it simply isn't rendered until its row is opened. All five/six of this app's directories (Org Users, Org Groups, Project Groups, Project Members, Server Admin's Access Review) now share this one shell rather than five bespoke ones.
...
## Pattern: directory table

`DirectoryTable` (`frontend/src/components/DirectoryTable.tsx`) is the shared shell for every "directory" named in the two patterns above — a `<table>` over caller-supplied, already-filtered/sorted/paginated rows, using `SortableHeader` for sortable columns and `LoadMoreButton` when the caller passes `total`/`onLoadMore`. It's deliberately thin, and deliberately doesn't absorb everything nearby:

- **No search box.** That's `FilterPanel`'s own `search`/`onSearchChange` header prop, composed beside `DirectoryTable` (`layout="side"`, narrow directories) or above it (`layout="top"`, wide ones — see "Pattern: filter panel placement — side vs. top" above) by every real call site — not inside the table component itself. This avoids the exact duplicate-search-input shape the table's own predecessor (`MemberRoleTable`) had before this component existed.
- **No `.side-grid`/`FilterPanel` wrapping.** Callers compose those themselves around `DirectoryTable`, whichever of the two placements fits; it renders inside the content column (or, for `layout="top"`, below the full-width filter bar) only, never wrapping either one itself.
- **No "add"/"create" control slot.** "New group," "Invite user," "Add member" — every such control stays a page-composed sibling *above* `<DirectoryTable>`, not threaded through a prop.
- **No fetch/mutate/merge logic.** 100% caller-owned: row merging (e.g. `kind: "user" | "invited"`), pagination mode, and every mutation handler stay in the page. Sorting is the same contract — `DirectoryTable` only calls `onSort(key)` on a header click; the caller owns `sort` state and decides how to re-sort (refetch or re-slice), then passes new `rows` back in.
- **The row-opening affordance is real, not a bare `<tr onClick>`.** `onRowClick` makes the first column's cell a real `<button>`; `rowHref` makes it a real, deep-linkable `<Link>` instead (`rowHref` wins if a caller somehow passes both — a `<Link>` is strictly more capable than a click handler for row navigation). Neither is required — a directory with no per-row detail (Server Admin's Access Review) simply leaves both unset, which is an ordinary consumer with two unset optional props, not a reason for a variant component.

**What stays bespoke on purpose, not folded into `DirectoryTable`:** every `SidePanel`'s own content — Project Groups' and Org Groups' panels show genuinely different fields, and `SidePanel` itself (Pattern "entity detail panel," above) is already the right shared abstraction for that; a further wrapper around its content would force a shape onto data that doesn't share one. Likewise the per-row role-editor control — `MultiSelectDropdown` for a genuinely multi-valued role vs. a plain `<select>` for a group's single `role` column — stays per-caller, since unifying these would repeat the exact mistake "Pattern: multi-select dropdown" above already warns against.

All six of this app's directories now share this one shell, landed across the three phases of the 2026-08-31 follow-up UX batch that touched it, with a later follow-up fix moving the three widest onto `layout="top"` (see "Pattern: filter panel placement — side vs. top" above). Two are page-composed the plain, narrow way — `.side-grid` + `DirectoryTable` + `FilterPanel` (`layout="side"`), side by side: Org Groups and Project Groups (`OrgAdminPage.tsx`/`ProjectAdminPage.tsx`, Phase B). Org Users (`OrgAdminPage.tsx`, Phase A) and Server Admin's Access Review (`ServerManagementPage.tsx`, Phase E) are page-composed the wide way instead — `FilterPanel` (`layout="top"`) above `DirectoryTable`, both full width, no `.side-grid`. The fifth and sixth — Project Admin's Members section and Org Admin's "Manage users" modal (Phase D) — both render the same shared `ProjectMembersTable` (`frontend/src/components/ProjectMembersTable.tsx`), which wraps `DirectoryTable` + `FilterPanel` (`layout="top"`) *inside itself* rather than leaving that composition to each page, specifically so the two call sites render and behave identically rather than each re-composing their own filters around a shared shell (see that component's own docstring). *Why it matters:* a design-review pass on the plan that built this component found four separate pages about to each hand-roll their own version of the exact shape `MemberRoleTable.tsx` had already proved out once — and that the *existing* Project Groups list wasn't a real `<table>` at all yet, just a `<button>` per row with no `SortableHeader`. One shared, deliberately narrow component stops a sixth bespoke directory from ever getting built, the same reasoning "Pattern: sortable column header" already applied to `SortableHeader` itself.

## Pattern: type-badged mixed rows in a single directory

Sometimes two (or more) genuinely different backend mechanisms surface the same *kind of thing* to the person managing a project — "another entry in the list of things granting access here" — and belong in one directory rather than a hidden, structurally-disconnected second location. PR5 of the members/groups directory rework plan is the first case: `ProjectAdminPage.tsx`'s Groups tab merges real `ProjectGroup` rows with `ProjectMemberSource` rows (project-to-project member mirroring) that previously sat on the Overview tab, reading to a user like "just another way to add a group" even though it's a different mechanism entirely. The fix is not a new shared component — it's the existing `kind`-discriminated union `DirectoryTable` row pattern (`ProjectMembersTable.tsx`'s own `member`/`invited` `Row` type is the precedent this one copies) plus one new visible signal: a "Type" column, badging each row with what it actually is.

```mermaid
flowchart LR
  A["DirectoryTable rows: GroupsTabRow[]"] --> B{"row.kind"}
  B -->|"group"| C["ProjectGroup row — Type: 'Project group'"]
  B -->|"member_source"| D["ProjectMemberSource row — Type: 'Project'"]
  C --> E["Click opens SidePanel (members, role, delete)"]
  D --> F["Read-only detail inline: mirror mode badge + Remove button"]
```

Read left to right: one array, one `kind` field deciding both the Type badge text and what the row can do — a real group keeps its existing click-to-open `SidePanel` affordance, a member-source row is read-only detail rendered directly in its own cells (no panel of its own to open, since there's nothing more to show than what a `ProjectMemberSource` already carries: its mirror mode/filter role and a Remove action).

Three choices worth naming so a second mixed directory adopting this pattern doesn't have to re-derive them:

- **The badge names what the row *is*, in this project's own terminology, not the backend's internal mechanism name.** A `ProjectMemberSource` row is badged "Project" (via `{Project}` terminology substitution), not "Org group" or "Member source" — it names *another whole project* this one mirrors members from, and calling it anything group-shaped would misdescribe the mechanism to the person reading it. Get this word choice from whoever reviews the merge, the way this PR's own plan review corrected an earlier draft's "Org group" label to "Project" specifically because it was wrong, not just inconsistent.
- **A virtual/read-only row kind never gains the real kind's affordances by accident.** A member-source row doesn't get a "click to open a panel" or an "add a member" control just because it now lives in the same table as rows that do — those affordances stay tied to `row.kind`, checked explicitly in every column's `render`, the same way `ProjectMembersTable.tsx`'s own Role column only offers a togglable checkbox for a `direct_role`-kind source and disables it for every other kind. Merging two mechanisms into one visual list is about *where* they're presented, not about pretending they're the same mechanism underneath.
- **Ordering favours the fixed, fully-loaded kind first.** Where one row kind is a small, always-fully-loaded set and the other paginates (`DirectoryTable`'s `total`/`onLoadMore`), the fixed kind renders first and stays pinned there — `LoadMoreButton` only ever appends more of the paginated kind after it, never reordering rows a person has already seen. Same precedent `ProjectMembersTable.tsx` already set (pending-invite rows ahead of the sortable/paginated member rows).

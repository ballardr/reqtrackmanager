...
The first three, plus both Phase 11 tools, take `organization_id` as a required parameter (`compliance_get_standard_version`/`compliance_list_requirements`/`compliance_list_requirement_mappings`/`compliance_get_standard_version_diff` also take `standard_id`/`version_id`, the latter two also a second version id and a `requirement_id` respectively) — Phase 4's scoping rule (see above): no implicit "current org," no cross-org aggregation. The five Phase 7/8/9/10 tools take `project_id` only, no `organization_id` at all — mirroring hand-written tools like `get_project(project_id)` — because they proxy to a *second*, project-scoped router the Compliance module registers (`get_project_router`, mounted at `/api/v1/projects/{project_id}/modules/compliance/...`) rather than its original org-scoped one; `build_mcp_tool_manifest` validates a tool's `path_template` against either of a module's two router prefixes, not just one. No mutating tool is declared for the standards API's publish/retire lifecycle, for changing a project's applicability/compliance-status assessments, for any evidence mutation (create/update/archive/revalidate/link/upload), for Phase 9's own `submit-for-approval`/`approve`/`reject` actions, for Phase 10's own review scheduling/completion/evidence-linkage endpoints, for Phase 11's own requirement-mapping create/archive/unarchive endpoints, or — most notably — for Phase 11's own project version-migration action (`POST .../project-compliance/{id}/migrate-version`), a significant mutation that touches every requirement row on an assignment and can downgrade in-flight/decided approvals — a same-day follow-up gave this endpoint an opt-in to carry an assessment forward across an explicitly-confirmed, org-vetted "replaced" mapping, which makes it more consequential, not less, so the same exclusion applies unchanged: Compliance's MCP surface stays deliberately read-only, the same cautious "add write tools narrowly and deliberately" default [Write mode](#write-mode) above already applies to this server's own hand-written tools. `approve`/`reject` specifically are also marked with this module's route-level `APPROVAL_ACTION_ROUTE_EXTRA` metadata, the same "approval stays human-only" mechanism [Write mode](#write-mode) describes for this server's own tools — so even a future session that mistakenly tried to declare an MCP tool for either would have it excluded from the manifest mechanically, not just by review.

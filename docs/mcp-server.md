...
A backend module can also contribute its own tools here, prefixed with that module's key (e.g. `compliance_list_standards`) — see [Module-contributed tools](#module-contributed-tools) below. The Compliance module registers six so far (all read-only), listed there.
...
**Compliance (compliance-module-plan.md Phase 6, extended in Phases 7 and 8) is the first module to use this**, contributing six read-only tools:

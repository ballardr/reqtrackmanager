...
**Module system follow-up (2026-09-05): registry-driven `Base.metadata`/schema automation.** `ModuleDefinition` gained two further optional fields removing the two remaining core-file touch points a new module previously needed. `models_import_path` (a dotted path to a module's own ORM models submodule) is imported by a new `app.modules.registry.import_all_module_models()`, called once by both `alembic/env.py` and `tests/conftest.py`, so `Base.metadata` includes every registered module's tables without a hand-written `import app.modules.<key>.models` line per module — inert with respect to any real database (it only registers an in-process table shape) and applies uniformly regardless of a module's source. `migrations_import_path` (a dotted path to a module exposing `run_migrations(connection) -> None`) is applied by `app.modules.registry.apply_external_module_migrations`, called from `app.migrations.run_migrations()` right after `alembic upgrade head` — but only for a module discovered via `Settings.allow_external_modules`'s entry-point/path sources, **never** for an `INSTALLED_MODULES` (first-party) module even if one declares it (checked structurally against the real `INSTALLED_MODULES` list, not trusted from the module's own declaration): first-party schema changes still go exclusively through a reviewed PR into `backend/alembic/versions/`, exactly as before. Each qualifying external module's migration runs in its own transaction (one `engine.begin()` per module) and must be idempotent, re-applied on every process start rather than tracked against its own revision history. This is a deliberate, narrower extension of the *existing* `ALLOW_EXTERNAL_MODULES` opt-in's scope — not a new, separate flag — so a module already trusted enough to load and run its own code can now also apply its own schema changes without an operator having to additionally hand-run a migration out of band; it does not change the trust boundary itself (`ALLOW_EXTERNAL_MODULES` off by default, CC6.8 rationale unchanged) or extend anything to first-party code. See `docs/decisions.md`'s two "Module system follow-up" entries (2026-09-05) for the full account, including an end-to-end proof against a real dropped-in fixture module and directory.
...
**Module-contributed MCP tools** (module system Phase 4) let a module declare its own `mcp-server/` tools — `ModuleDefinition.mcp_tools`, a tuple of `McpToolDefinition(name, description, method, path_template, params)` where `params` is a list of `{"name", "type", "required", "in", "description"}` dicts (a display/JSON-Schema hint for the calling AI assistant, not itself security-enforced). `app.modules.registry.build_mcp_tool_manifest()` turns these into the manifest `GET /api/v1/system/modules/mcp-tools` (gated by plain `get_current_user` — normal bearer-token authentication, deliberately no exemption) returns, re-deriving or verifying every security-relevant part mechanically rather than trusting the module's own declaration, the same "verify, don't trust" principle Phase 3's `get_frontend_manifest` already applies to a Tier B `frame_url`: the registered tool name is always `f"{module.key}_{tool.name}"`, regardless of what local `name` a module declares, so one module can never claim or collide with another's or core's tool name; `mutates` is derived purely from `method` (`"GET"` → `False`, else `True`); `path_template` must fall inside one of the declaring module's own router mount prefixes (`get_router()`'s, and `get_project_router()`'s too if the module declares one — see Phase 7 below, the first module with both) and resolve to a real route on *that same router* with a matching method, checked directly against that router's own `.routes` (a `FastAPI`/Starlette `APIRoute`'s `.path` is already the full, prefix-combined path the instant it's added to an `APIRouter(prefix=...)`, before that router is ever mounted into the running app — so this check needs no live `app.routes` lookup and works identically whether or not the module's router has actually been included yet); and whether the resolved route is an approval/decision-type action is read from that route's own `openapi_extra` metadata (`APPROVAL_ACTION_EXTRA_KEY = "x-approval-action"`, a marker its own author sets via `openapi_extra=APPROVAL_ACTION_ROUTE_EXTRA`, reviewed the same way the endpoint itself is) rather than from the module's `McpToolDefinition` — any tool resolving to such a route is excluded from the manifest entirely. A `path_template` outside its module's own router, a `path_template`/`method` pair with no matching route, or a single malformed param dict is excluded (and logged) individually — one bad declaration never takes down a module's other, valid tools.

`mcp-server/server.py` fetches this manifest **lazily and authenticated** (never at an unauthenticated boot-time call): a `ModuleToolsRefreshMiddleware` triggers `_maybe_refresh_module_tools()` on every `tools/list`/`tools/call`, which refetches at most once per `MODULE_TOOLS_REFRESH_SECONDS` (default 600s) using whichever connecting session's own already-presented bearer token is on hand, skipping the cycle entirely (not erroring) if none is present or the backend is momentarily unreachable. `_apply_module_tool_manifest` diffs the fetched specs against what's currently registered — removing a tool no longer present, registering (via `_register_declarative_tool` → `mcp.add_tool`) one that's new or changed, and touching neither FastMCP call for a tool whose spec is byte-for-byte unchanged since the last refresh, so an unchanged module's tools don't generate a duplicate-registration warning log every refresh cycle. A mutating (`mutates: True`) tool is only ever included in what gets registered when this server's own `MCP_WRITES_ENABLED` is set — the same gate its hand-written write tools already use, applied generically. Each registered tool is a `_DeclarativeModuleTool` (a `fastmcp.tools.Tool` subclass) whose `run()` builds one `_call_backend` proxy call from the spec's params — a path-location value is URL-encoded before being substituted into `path_template` (this server has no way to know ahead of time which params are UUIDs and which aren't, so every path value is encoded regardless, the same "encode by construction, not by cases" reasoning `_get_org_login_info` already uses for `?org=<slug>`), query-location values become query params, and body-location values are assembled into the JSON body — no module code ever runs inside `mcp-server`'s own process; every declarative tool is a plain HTTP proxy call, so no packaging change is needed there either.

**Compliance module — data model** (compliance module Phase 5) is the first real first-party module built on top of the four module-system phases above: `app.modules.compliance.module.MODULE_DEFINITION` registers `key="compliance"`, `default_enabled=True` (§1's "enabled by default"), and the two module-contributed roles §3/§26 call for — `compliance_manager` (org-scoped) and `compliance_officer` (project-scoped) — via `ModuleDefinition.roles`, without adding either to the core `OrgRole`/`ProjectRole` enums. Phase 5 contributed no HTTP endpoints or MCP tools yet; both landed in Phase 6 (Standards Management API) — `app.modules.compliance.router` mounts full CRUD for standards, versions, requirements, required actions, and an organisation-scoped action-type vocabulary at `/api/v1/orgs/{organization_id}/modules/compliance`, gated by `require_module_role("compliance", "compliance_manager")` for writes (composing, unreimplemented, with the existing `is_server_admin`/`OrgRole.ORG_ADMIN` override) and the weaker `require_org_module_enabled("compliance")` for reads, and declares three read-only MCP tools (`compliance_list_standards`, `compliance_get_standard_version`, `compliance_list_requirements`) via `ModuleDefinition.mcp_tools`. Publishing a version (draft -> published) makes its requirements and required actions immutable at the API layer (409 on create/update/delete/reorder); retiring (from draft or published) is terminal; no version is ever deleted. Creating a version optionally deep-copies ("clones") a prior version's full requirement tree and required actions forward, since §4 expects new versions to build on existing content rather than starting from scratch each time. Every mutation is logged via `services.audit.log_event`. See `docs/decisions.md`'s "Compliance module plan, Phase 6" entry and `docs/compliance-module-plan.md`'s "Phase 6 notes" for the full account, including every path/schema judgment call. Its data model (`app.modules.compliance.models`, its own package rather than `app.models`, keeping the module's domain self-contained) follows §25's conceptual shape: `ComplianceStandard` (an organisation-level, reusable definition — §31: "not a Project") has `ComplianceStandardVersion` children (§4's versioning requirement) following the `Requirement`/`RequirementVersion` identity-row-plus-versioned-children shape, but *not* that pair's `valid_from`/`valid_to` supersession semantics — every standard version stays independently addressable indefinitely (ordered/gated instead by `version_number` and a `draft → published → retired` `status`), since different projects may deliberately stay pinned to different past versions forever (§4's own worked example). Each version has `ComplianceRequirement` children, self-referential via a plain `parent_requirement_id` foreign key (no ORM `relationship()`, mirroring `Project.parent_project_id`'s precedent rather than `RequirementVersion`'s) for §5's section/subsection hierarchy, each in turn owning zero or more `ComplianceRequiredAction` rows (§6) — a plain one-to-many, not a many-to-many link table like `RequirementAction`/`RequirementActionLink`, since §25's diagram places required actions directly under compliance requirements with no separate mapping table. `ComplianceActionTypeDefinition` is an organisation-scoped, extensible required-action-type vocabulary mirroring `ActionTypeDefinition`'s existing pattern at org rather than project scope, since a required action's type is chosen within an organisation-level standard. Per §31's "a Compliance Requirement must not contain the compliance state of a project," none of these five tables carries any project-specific assessment state (status, applicability, evidence, approval) — that lands in Phase 7 on top of these definitions, in a `ProjectCompliance`/`ProjectComplianceRequirement` pair that references, but never duplicates, them. Compliance-specific vocabulary enums (`ComplianceStandardVersionStatus`, plus `ComplianceStatus`/`ComplianceApplicability`/`ComplianceApprovalState`, defined now but not yet referenced by any model until Phases 7/9) live in `app.modules.compliance.enums`, not `app.models.enums`, for the same self-containment reason. See `docs/decisions.md`'s "Compliance module plan, Phase 5" entry for the full account, including how the module's own `MODULE_DEFINITION`-registration import cycle (a first-party module's `module.py` imports `ModuleDefinition` from `app.modules.registry` to construct its definition, so `registry.py` importing that definition back would be circular) was resolved by moving the wiring into `app/modules/__init__.py` instead.

```mermaid
erDiagram
    ORGANIZATION ||--o{ COMPLIANCE_STANDARD : owns
    COMPLIANCE_STANDARD ||--o{ COMPLIANCE_STANDARD_VERSION : has
    COMPLIANCE_STANDARD_VERSION ||--o{ COMPLIANCE_REQUIREMENT : contains
    COMPLIANCE_REQUIREMENT |o--o{ COMPLIANCE_REQUIREMENT : parent_of
    COMPLIANCE_REQUIREMENT ||--o{ COMPLIANCE_REQUIRED_ACTION : requires
    ORGANIZATION ||--o{ COMPLIANCE_ACTION_TYPE_DEFINITION : defines
    COMPLIANCE_ACTION_TYPE_DEFINITION ||--o{ COMPLIANCE_REQUIRED_ACTION : types
    PROJECT ||--o{ PROJECT_COMPLIANCE : assigned
    COMPLIANCE_STANDARD_VERSION ||--o{ PROJECT_COMPLIANCE : assigned_version
    PROJECT_COMPLIANCE ||--o{ PROJECT_COMPLIANCE_REQUIREMENT : assesses
    COMPLIANCE_REQUIREMENT ||--o{ PROJECT_COMPLIANCE_REQUIREMENT : assessed_by
    PROJECT_COMPLIANCE_REQUIREMENT ||--o{ COMPLIANCE_REQUIRED_ACTION_ASSESSMENT : assesses
    COMPLIANCE_REQUIRED_ACTION ||--o{ COMPLIANCE_REQUIRED_ACTION_ASSESSMENT : assessed_by
    PROJECT ||--o{ COMPLIANCE_EVIDENCE : has
    COMPLIANCE_EVIDENCE ||--o{ COMPLIANCE_EVIDENCE_REVALIDATION : history
    COMPLIANCE_EVIDENCE ||--o{ COMPLIANCE_EVIDENCE_FILE : attaches
    COMPLIANCE_EVIDENCE ||--o{ COMPLIANCE_EVIDENCE_REQUIREMENT_LINK : supports
    PROJECT_COMPLIANCE_REQUIREMENT ||--o{ COMPLIANCE_EVIDENCE_REQUIREMENT_LINK : supported_by
    COMPLIANCE_EVIDENCE ||--o{ COMPLIANCE_EVIDENCE_ACTION_LINK : supports
    COMPLIANCE_REQUIRED_ACTION_ASSESSMENT ||--o{ COMPLIANCE_EVIDENCE_ACTION_LINK : supported_by
    COMPLIANCE_STANDARD |o--o{ COMPLIANCE_REVIEW : reviews
    PROJECT_COMPLIANCE |o--o{ COMPLIANCE_REVIEW : reviews
    COMPLIANCE_EVIDENCE ||--o{ COMPLIANCE_REVIEW_EVIDENCE_LINK : supports
    COMPLIANCE_REVIEW ||--o{ COMPLIANCE_REVIEW_EVIDENCE_LINK : supported_by
```

`COMPLIANCE_REQUIREMENT`'s self-referential `parent_of` relationship is the same `parent_requirement_id` foreign key described above, drawn as one-to-many-optional (`|o--o{`) since a requirement's parent is nullable (a top-level requirement has none). `COMPLIANCE_STANDARD`/`COMPLIANCE_STANDARD_VERSION` each also carry `owner_id`/`creator_id`/`created_by`/`published_by`/`retired_by`/`archived_by` foreign keys to `USER`, omitted from this diagram for the same crossing-lines reason `PENDING_INVITE.invited_by` and `USER_ORG_ROLE`/`USER_SERVER_ROLE`'s `granted_by` already are elsewhere in this document. `PROJECT_COMPLIANCE`/`PROJECT_COMPLIANCE_REQUIREMENT`/`COMPLIANCE_REQUIRED_ACTION_ASSESSMENT` (Phase 7, described below) likewise carry several `USER` foreign keys (`assigned_by`, `archived_by`, `assessed_by`, `applicability_set_by`, `assignee_id`, `completed_by`, plus Phase 9's `approval_decided_by` on `PROJECT_COMPLIANCE_REQUIREMENT`) omitted here for the same reason. `COMPLIANCE_EVIDENCE`/`COMPLIANCE_EVIDENCE_REVALIDATION`/`COMPLIANCE_EVIDENCE_FILE`/`COMPLIANCE_EVIDENCE_REQUIREMENT_LINK`/`COMPLIANCE_EVIDENCE_ACTION_LINK` (Phase 8, described below) likewise carry `USER` foreign keys (`provided_by`, `archived_by`, `revalidated_by`, `linked_by`) and `COMPLIANCE_EVIDENCE_FILE` also carries a `FILE_ASSET` foreign key (`file_id`), both omitted for the same reason. `COMPLIANCE_REVIEW` (Phase 10, described below) has exactly one of `standard_id`/`project_compliance_id` set (a `CHECK` constraint, not drawn as two separate optional relationships colliding — both `|o--o{` edges above are individually optional for exactly this reason) and also carries `owner_id`/`created_by`/`completed_by` `USER` foreign keys, omitted for the same reason as every other entity in this diagram; `COMPLIANCE_REVIEW_EVIDENCE_LINK` likewise carries a `linked_by` `USER` foreign key, also omitted.

**Compliance module — project compliance assignment & assessment** (compliance module Phase 7) adds the project-specific assessment layer on top of Phase 5/6's organisation-level standard definitions: `ProjectCompliance` (`app.modules.compliance.models`) is a project's assignment to one specific, always-`PUBLISHED` `ComplianceStandardVersion` (§7) — a `DRAFT` version's content isn't fixed yet, and a `RETIRED` one is, by its own enum docstring, "no longer assignable to **new** projects." `ProjectComplianceRequirement` is one project's per-requirement assessment (§8) — applicability, compliance status, a single `justification` column serving both §9's mandatory Not-Applicable justification and §16's mandatory Non-Compliant rationale (never needed simultaneously), notes, and actor+timestamp pairs for each of those two independent decisions, plus an `approval_state` column reserved for Phase 9's workflow (present now, at `NOT_ASSESSED`, purely so this phase's own §20 overall-status calculation — which folds approval state in — doesn't have to be revisited when Phase 9 lands). `ComplianceRequiredActionAssessment` is one project's per-required-action assessment (§6/§25), shaped as a completion overlay (`is_completed`/`completed_at`/`completed_by`, mirroring `Requirement.completed_at`'s own shape) rather than reusing `ComplianceStatus`. Every requirement's and required action's assessment row is materialised in full at assignment time (`app.modules.compliance.service.materialize_assessment_rows`), never lazily — a published version's content is immutable, so there is nothing to reconcile into these rows later. §9's hierarchical applicability (parent Not Applicable ⇒ children inherit; a child may explicitly override back to Applicable) is resolved on read, never stored, by `service.resolve_applicability_for_version` — a single pass over one version's requirement tree that classifies each row's effective value as `EXPLICIT` (a user-set or still-default value with no conflicting ancestor), `INHERITED` (no explicit decision, nearest ancestor is Not Applicable), or `OVERRIDDEN` (explicitly Applicable despite a Not Applicable ancestor). §20's overall-status calculation — required by the plan to be exactly and publicly defined — is `service.summarize_project_compliance`: total/applicable/not-applicable counts and a per-`ComplianceStatus` breakdown (Not Applicable rows excluded from every percentage/status calculation), a compliance percentage (`compliant / applicable * 100`, `100.0` when nothing is applicable rather than a division by zero), a `has_non_compliant` flag shown regardless of how high the percentage is, an `overall_compliance_state` (`compliant`/`non_compliant`/`in_progress`/`not_applicable`), and a **separately-reported** `overall_approval_state` (a "worst state wins" reduction over every applicable row's own `approval_state` — `REJECTED` > `REQUIRES_REASSESSMENT` > `PENDING_APPROVAL` > `ASSESSED` > `NOT_ASSESSED` > `APPROVED`, so the overall value is `APPROVED` only when every applicable row is), never folded into `overall_compliance_state` — per §20's explicit "a project may have 100% compliant requirements but still have an overall status of 'Pending Approval'."

This phase is also the first to give a module **two** routers: `app.modules.compliance.router` (org-scoped, unchanged prefix, gaining standard-to-project assignment/archival endpoints — assigning a standard is a Compliance Manager decision, per §26) and a new `app.modules.compliance.project_router` (project-scoped, `/api/v1/projects/{project_id}/modules/compliance/...`, no `organization_id` anywhere in its paths) for day-to-day viewing/assessment, registered via a new optional `ModuleDefinition.get_project_router` field mounted by `app.main`'s loop exactly like `get_router`. This split exists because `compliance_get_project_status`/`compliance_list_non_compliant_requirements` (this phase's two new, read-only MCP tools) must declare `project_id` as their only path parameter, mirroring hand-written tools like `get_project(project_id)` — impossible for a route that also carries `{organization_id}`. `build_mcp_tool_manifest` was extended (not reimplemented) to validate a tool's `path_template` against *either* of a module's router prefixes, using whichever router's `.routes` the path actually resolves against. Mutating endpoints (applicability, assessment, required-action-assessment updates) are gated by `require_module_role("compliance", "compliance_officer")`, which already composes `is_server_admin`/`ProjectRole.PROJECT_MANAGER` (Phase 2's own design, unreimplemented here); reads are gated by the weaker `require_project_module_enabled("compliance")`. A requirement's own assessment history (§8/§11/§16) is served by a new, generic `schemas.audit.AuditEventOut` over the existing `AuditEvent` table — filtered by `entity_type="project_compliance_requirement"` — the first time this codebase has exposed the audit trail over HTTP at all, rather than a second, compliance-specific history mechanism. See `docs/decisions.md`'s "Compliance module plan, Phase 7" entry and `docs/compliance-module-plan.md`'s "Phase 7 notes" for the full account.

**Compliance module — evidence** (compliance module Phase 8, §13-§15) adds project-scoped supporting evidence on top of Phase 7's assessment layer, added to `project_router.py` rather than a third router (evidence needs no new MCP-tool-scoping shape `get_project_router` doesn't already provide). `ComplianceEvidence` (`app.modules.compliance.models`) belongs to a project directly, not to one specific `ProjectCompliance` assignment — a single row may support requirements/required actions across more than one of a project's standard assignments at once (§13's "a single piece of evidence should be capable of supporting multiple compliance requirements"). That multi-linkage is genuine many-to-many, via `ComplianceEvidenceRequirementLink`/`ComplianceEvidenceActionLink`, pointing at the project-specific `ProjectComplianceRequirement`/`ComplianceRequiredActionAssessment` rows — not the reusable `ComplianceRequirement`/`ComplianceRequiredAction` definitions — the same §31 "state belongs to the project-specific assessment layer" principle Phase 7 already established, extended one concept further. File attachments reuse `services.files.upload_file` (§13's "reuse ReqTrackManager's existing attachment/file mechanisms") via a new join table, `ComplianceEvidenceFile`, in this module's own package rather than `app.models.file` — the exact shape of `RequirementFile`. §15's revalidation is modelled as a genuinely append-only `ComplianceEvidenceRevalidation` table (no `updated_at`, insert-only, mirroring `RequirementFile`'s own convention) rather than a stored history field, so a second revalidation can never overwrite the first's own "previous expiry" value; `ComplianceEvidence.expiry_date` itself is updated in place to the *current* effective expiry, with each transition's previous/new value also recorded via `services.audit.log_event`. §14's "valid/expiring/expired" state (`ComplianceEvidenceValidityState`: `no_expiry`/`valid`/`expiring_soon`/`expired`) is computed at read time by `service.compute_evidence_validity_state` — never stored, so it can't drift — using a plain module-level warning-window constant (`EVIDENCE_EXPIRY_WARNING_DAYS = 30`, not yet a configurable per-project/org setting; §14 names no specific lead time, and Phase 10 is where this module's actual notification delivery is built, the natural place to promote this to a setting if that phase needs one). `GET .../expiring-evidence` (the `compliance_list_expiring_evidence` MCP tool) excludes archived evidence — "no longer applicable" (§13) and "no longer valid" (§14) are deliberately different questions, per `ComplianceEvidenceValidityState`'s own docstring.

This phase also adds a small, generic module-system extension: `ModuleDefinition.resolve_file_owner_project_id` (`app.modules.registry`), an optional hook a module implements to resolve one of its own file-link tables to an owning project id, and `resolve_module_file_project_id`, which the core, module-agnostic `GET /api/v1/files/{id}` download endpoint (`app.routers.files.download_file`) now calls — after every core attachment-type check (requirement/action/comment) — as an additional fallback, without that router importing anything from any specific module. Compliance's own `resolve_evidence_file_project_id` (`app.modules.compliance.service`) is the first, and only, implementation so far. See `docs/decisions.md`'s "Compliance module plan, Phase 8" entry and `docs/compliance-module-plan.md`'s "Phase 8 notes" for the full account.

**Compliance module — approval / sign-off workflow** (compliance module Phase 9, §12, §16, §27) activates the `approval_state` state machine Phase 7 reserved a column for: `ProjectComplianceRequirement` gains `approval_decided_at`/`approval_decided_by`/`decision_note` (§12's approval timestamp/actor and a rationale serving both an approval and a rejection decision, the same "one column, meaning depends on which transition is happening" shape `justification` already established), and `project_router.py` gains three actions enforcing `NOT_ASSESSED → ASSESSED → PENDING_APPROVAL → APPROVED/REJECTED` ordering (409 otherwise): `submit-for-approval`, `approve`, `reject` (the last requiring a non-blank `decision_note`, mirroring §16's mandatory-rationale pattern). No new history mechanism — §12's "approval/sign-off history" is the same `AuditEvent`/`get_requirement_history` trail Phase 7 already exposed. Two automatic transitions, both in `service.py`, cover §12/§16/§27's "must not silently remain Approved": `advance_approval_state_on_assessment` always moves a row to `ASSESSED` when its compliance status is reassessed (from *any* prior state, including `APPROVED` — a fresh assessment is itself the reassessment, so it must not land on `REQUIRES_REASSESSMENT`, which means "not yet reassessed"); `invalidate_approval_if_in_flight` downgrades `PENDING_APPROVAL`/`APPROVED` to `REQUIRES_REASSESSMENT` for a *material change without an accompanying reassessment* — an applicability change, or (via a new `find_pcrs_linked_to_evidence` evidence→PCR lookup spanning both direct and required-action-assessment links) evidence being archived or revalidated. One more read-only MCP tool, `compliance_list_pending_approvals(project_id)`; `approve`/`reject` are marked with `app.modules.registry.APPROVAL_ACTION_ROUTE_EXTRA` so Phase 4's manifest-builder exclusion catches them mechanically even though no tool is declared for them at all — belt-and-braces, not the only defence. See `docs/decisions.md`'s "Compliance module plan, Phase 9" entry and `docs/compliance-module-plan.md`'s "Phase 9 notes" for the full account.

**Compliance module — scheduled reviews + notifications** (compliance module Phase 10, §17, §18, §28) adds `ComplianceReview` (`app.modules.compliance.models`) — a scheduled review of either a `ComplianceStandard` itself or one project's `ProjectCompliance` assignment (a `CHECK` constraint enforces exactly one owner) — plus `ComplianceReviewEvidenceLink`, the same many-to-many evidence-linkage shape Phase 8 established, pointing at the project-specific assessment layer. A review's own "upcoming/due/overdue" state (`service.compute_review_schedule_state`) is computed at read time from `next_due_date`, never stored, the same "can't drift" reasoning `compute_evidence_validity_state` already established; completing a `SCHEDULED` review (`service.complete_review`) retains it as permanent history and, when `recurrence_days` is set, creates a **new** `SCHEDULED` row for the next cycle rather than reopening the completed one — §17's own "completed reviews must be retained" is satisfied by this chain of rows, not a second history table. Four new APScheduler jobs (`app.modules.compliance.scheduler`) sweep for evidence expiry, review due/overdue, required-action due/overdue, and compliance-target-date approaching/exceeded — each idempotent per notification via a `*_reminder_sent_at`/`*_notified_at` timestamp pair on the owning row (mirroring `RequirementVersion.review_reminder_sent_at`'s own Massif-era convention), reset whenever the underlying date is rescheduled. Six more notifications are event-driven, wired directly into the endpoints that cause them (a Non-Compliant assessment, submit-for-approval, reject, an applicability/evidence change invalidating an in-flight approval, a new standard version being published, and a new project assignment). Fourteen new `NotificationType` members cover every event §18 lists, added to the shared, module-agnostic `Notification`/`NotificationPreference` tables (this phase also fixes a pre-existing drift where `frontend/src/api/types.ts`'s `NotificationType` union was missing two values `frontend/src/i18n/strings.ts` already had).

This phase also adds a second small, generic module-system extension, mirroring Phase 8's `resolve_file_owner_project_id` precedent: `ModuleDefinition.scheduled_jobs` (`app.modules.registry.ModuleScheduledJob`) lets a module declare its own APScheduler jobs, registered generically by `app.services.scheduler.start_scheduler` (job-id-prefixed with the declaring module's own key) alongside that file's own two core jobs — without that core file importing anything module-specific. Compliance's own four sweeps are the first, and so far only, jobs registered this way. See `docs/decisions.md`'s "Compliance module plan, Phase 10" entry and `docs/compliance-module-plan.md`'s "Phase 10 notes" for the full account.
...
The database is organised into 69 tables (as of this revision), grouped by which of the six diagrams above they belong to. This list is intentionally exhaustive rather than illustrative — treat a change that adds a table without updating this list the same as any other documentation drift. (Two corrections made in passing while adding `org_group_project_roles` in an earlier revision: this list had drifted to 46 while the schema actually had 47 — `project_member_sources`, present in the ER diagram above and in every migration/model, had been missing from this list entirely; added back then alongside that new table rather than left as a second, unrelated gap. A third, unrelated drift found and fixed while adding `project_group_roles` for PR7 below: the "One diagram covering all N tables" sentence just above the five-diagram split had been left at a stale "45" through at least the previous two additions. A fourth drift, found and fixed while adding the module system's tables for Phase 1: `server_roles` (Phase 0) had never been added to this list or the identity diagram above at all — this revision adds it alongside Phase 1's own two new tables rather than leaving it as a second, unrelated gap, the same pattern as the first correction above. A fifth and sixth, found while adding compliance module Phase 5's five tables and fixed in the same pass rather than left as further unrelated gaps: the sentence above still said "five diagrams" even after Phase 5 added a sixth ER diagram just above; and Phase 2's own `module_role_definitions`/`user_module_roles` — present in the ER diagram above and described in prose under "Modular Feature System" — had never actually been added to this exhaustive list at all, discovered by cross-checking this list's count against a live migrated database's real table count rather than assumed correct. A seventh, larger drift found and fixed while adding compliance module Phase 10's own two new tables: the "Compliance module" group above had been left at only Phase 5's five tables ever since Phase 7 — the eight tables Phases 7 and 8 actually added (`project_compliances`, `project_compliance_requirements`, `compliance_required_action_assessments`, `compliance_evidence`, `compliance_evidence_revalidations`, `compliance_evidence_files`, `compliance_evidence_requirement_links`, `compliance_evidence_action_links`) were each individually described in this document's own prose sections and present in the ER diagram above, but never carried over into this exhaustive list — discovered the same way the sixth drift was, by cross-checking this list's count against a live migrated database's real table count (69, not the 61 this list would have summed to with only Phase 10's own two tables added on top of the stale five) rather than assumed correct.)
...
**Platform administration, notifications, and audit**: `custom_field_definitions`, `report_templates`, `personal_access_tokens`, `notifications`, `notification_preferences`, `login_events`, `audit_events`, `favorite_projects`, `stage_review_responses`, `server_settings`, `organization_module_entitlements`, `organization_modules`, `module_role_definitions`, `user_module_roles`.

**Compliance module** (compliance module Phases 5/7/8/10 — Phase 9 added columns only, no new tables): `compliance_standards`, `compliance_standard_versions`, `compliance_requirements`, `compliance_required_actions`, `compliance_action_type_definitions` (Phase 5); `project_compliances`, `project_compliance_requirements`, `compliance_required_action_assessments` (Phase 7); `compliance_evidence`, `compliance_evidence_revalidations`, `compliance_evidence_files`, `compliance_evidence_requirement_links`, `compliance_evidence_action_links` (Phase 8); `compliance_reviews`, `compliance_review_evidence_links` (Phase 10).

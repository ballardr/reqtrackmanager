...
A `REQUIREMENT_ACTION` (e.g. a review or a test a requirement needs performed against it) has its own project-scoped identity — its own `unique_code`, generated the same way `REQUIREMENT.unique_code` is — rather than belonging to exactly one requirement, so `REQUIREMENT_ACTION_LINK` is a genuine many-to-many join: one action (e.g. a single wind-tunnel test) can satisfy several requirements at once, and unlinking it from one requirement never deletes it or affects any other requirement still linked to it. `ACTION_TYPE_DEFINITION` is project-scoped (not org-scoped, unlike `PROJECT_STATUS_DEFINITION`/`REQUIREMENT_LINK_TYPE_DEFINITION` above) — matching `CUSTOM_FIELD_DEFINITION`'s existing project-scoping — per an explicit product decision to keep the door open for a future project-nesting feature to resolve action types through a parent project, without that being built yet (see decisions.md). `REQUIREMENT_ACTION_FILE` is a direct-attachment table, structurally identical to `REQUIREMENT_FILE` one diagram over, just scoped to an action instead of a requirement — an action's own discussion-thread attachments instead reuse the generic `REVIEW_COMMENT`/`COMMENT_FILE` machinery from the next diagram (a third `target_type`, alongside requirement and change request), not a fourth parallel comment system.

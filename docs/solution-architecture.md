...
Phases 3 and 4 of the module system plan build on this same registry shape without further changing it: a two-tier frontend integration model (installed-at-build-time modules using real shared components directly, versus a sandboxed-iframe-plus-host-UI-bridge path for genuinely dynamic ones), and declarative, single-REST-call module-contributed `mcp-server/` tools. `ModuleDefinition` already carries placeholder fields (`frontend_manifest`, `mcp_tools`) for these so the dataclass's shape doesn't need a breaking change as each phase lands.

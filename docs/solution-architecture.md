...
`PROJECT |o--o{ PROJECT : parent_of` is `PROJECT.parent_project_id`, a nullable self-referential foreign key (`ondelete="SET NULL"`) — read it as "a project has zero-or-one parent, and zero-or-many children," the same self-join shape `ORG_GROUP_MEMBER.member_org_group_id` already uses for nested org groups, with the identical no-DB-level-cycle-constraint tradeoff (cycles are prevented at the application layer, `would_create_project_cycle`, since a `CHECK` constraint can't express "no cycle anywhere in this table's transitive closure"). `PROJECT_MEMBER_SOURCE` (table `project_member_sources`) is a plain join table with two FKs to `PROJECT`, both `ondelete="CASCADE"`, drawn as two separate relationships (`consumes_from`/`consumed_by`) because the two ends mean opposite things and are authorized by opposite sides — see the "Hierarchical projects" entry in [decisions.md](decisions.md) for the full RBAC-cascade design (forward mirroring via `parent_project_id` + `Project.role_inheritance_mode`, reverse consumption via this table) and why that authorization asymmetry exists. Neither mechanism is itself a new access-control *primitive* — both ultimately still resolve down to the same `USER_PROJECT_ROLE`/`PROJECT_GROUP_MEMBER` rows shown above, just reached by walking `parent_project_id`/`project_member_sources` first; `services/rbac.py` remains the single place that combines all of it into one project's effective role set.

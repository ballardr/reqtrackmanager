...
| Extend terminology overrides to actually cover their own surface | L | Not started | Highest-value item relative to its severity — 4 of 6 configurable terms are consumed nowhere today. Explicitly deferred to last among the remaining roadmap items, per direct user instruction — when tackled, it's to be the full architectural fix (`strings.ts` itself becomes terminology-aware, all 6 terms wired up), not a partial patch of the 2 currently-working terms' call sites. |

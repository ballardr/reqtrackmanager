...
| Add `aria-label` to the ~20 unlabelled icon buttons | S | Not started | No new component needed — the correct pattern already exists elsewhere (`RequirementsPage.tsx`'s reorder buttons); apply it to `OrgAdminPage.tsx`/`ProjectAdminPage.tsx`'s gaps. |
| Swap `RichTextEditor`'s `window.prompt()` for the app's own `Modal` | S | Not started | One call site, one file. |
| Reset affordance for text-based branding overrides | S | Not started | Frontend-only — the backend already reverts on an empty string; add a status pill + explicit reset link. |
...
| Shared, accessible `Tabs` component with arrow-key navigation | M | Not started | Migrate the 3 hand-rolled bars; closes the ARIA gap and the keyboard-navigation gap in one pass. |
| `Modal`: add a focus trap and focus restoration on close | S | Not started | The component's ARIA roles are already correct — two remaining behavioural gaps, contained to one file since it's used everywhere already. |
| Shared `SidePanel` and `Popover` (content) components | M | Not started | Prerequisite for the create-flow and bulk-entry-point items below. |
| Shared Modal-based confirm + type-to-confirm variant | M | Not started | Replaces all four current patterns per the two-tier rule. Highest-severity item relative to its footprint — touches at least 15 unconfirmed-delete call sites. |
| Shared Toast component, wired into every mutation | M | Not started | Success feedback exists in one place today; fixes the other 90%+ of create/save/delete/vote actions. |
...
| One entry point for single vs. bulk requirement/change-request creation | M | Not started | UI consolidation only — the CSV import wizard and its endpoint are unchanged. |
...
| Nav-rail entry point to `/orgs` (org administration) | S | Not started | Corrected from an earlier draft's "org switcher" proposal, which was the wrong pattern — see [Navigation & the orphan page](#navigation--the-orphan-page). Just a visible link for org admins; no new state, no new component. |

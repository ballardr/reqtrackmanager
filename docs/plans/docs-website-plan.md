...
**Last updated:** 2026-09-15 (Phase 11 complete — plan closed).
...
| 10 | Polish, cross-linking, search, broken-link gate, final QA — including a visual QA pass | [x] Complete |
| 11 | Content review: verbosity/redundancy pass + cold-reader "what is this and why would I use it" audit | [x] Complete |
...
## Phase 11 — Content review: verbosity/redundancy pass + cold-reader audit

An eleventh phase, added after Phase 10 closed the plan, in response to the user's follow-up ask for a full content-quality review: is any page too verbose, does the site make sense read cold, and — treated as its own explicit question — does a reader who knows nothing about the project come away from the Introduction understanding what ReqTrackManager is, why it exists, what it does, why they'd want it, how to use it, and when it's useful.

**Method:** four parallel review passes, one per pair of adjacent sidebar sections (Concepts+Installation & Deployment; Core Features+Workflows; Modules+API & Integrations; Enterprise & Security+Reference), each reading every file in its two directories in full and reporting concrete, quoted findings rather than a subjective score — plus a direct read of `introduction/overview.md` and `introduction/use-cases.md` against the cold-reader question, since those two pages carry that question's full weight.

**Overall verdict, cold-reader question:** the Introduction section already does this well and needed no changes — *Overview* states what the product is, who it's for, and where to go next in its first paragraph; *Use cases* grounds five concrete scenarios (hardware/firmware team, regulated-software team, spreadsheet-outgrowing team, compliance-tracking team, multi-project org) in real, cross-linked features rather than restating the feature list in different words. This matches the plan's own Phase 2 spec and the "Decided by: Agent" reasoning recorded there.

**Overall verdict, verbosity:** the site is *not* systemically bloated — most pages are already tightly written, well-differentiated from their neighbours (Core Features' reference prose vs. Workflows' imperative steps), and free of the test-plan-language smell this plan's own "Notes for whoever resumes a phase" section warned against. The real, fixable problems found were narrower and are listed below by category, each now fixed:

- **Redundant "why this matters" paragraphs restating a point already made earlier on the same page** — `concepts/discussions-notifications-and-audit.md` (trimmed the "Keeping these separate means..." paragraph to keep only its one non-redundant point), `concepts/custom-fields-terminology-and-templates.md` (folded a two-sentence "Why this matters" header into the preceding section rather than giving it its own heading).
- **Near-verbatim duplication between a Concepts page and its own detail page** — `concepts/organisations-and-projects.md`'s org/project role list was restated near word-for-word in `concepts/roles-groups-and-access-control.md`; condensed the former to name the roles once and point at the latter for the full description.
- **A "common mistake" callout in `installation-deployment/tls-and-reverse-proxy.md` that stated its own conclusion three times** (lead-in, the math, a closing restatement) — tightened to lead-in + math, dropped the restatement; also removed an internal QA-methodology sentence ("this exact config was verified against...") that read as a testing note bleeding into user-facing deployment docs rather than instructive content.
- **Near-verbatim duplication between a Core Features page and its Workflows counterpart**, three instances — file-attachment mechanics (`core-features/file-attachments-and-shared-resources.md` vs. `workflows/files-comments-and-history.md`), stage deletion/reassignment rules (`core-features/stages-and-baselining.md` vs. `workflows/stages-baselining-and-reviews.md`), and the bell-icon notification-type list (`core-features/notifications-and-email.md` vs. `workflows/notifications.md`). In each case the Workflows page (task-oriented, "how do I") was trimmed to a short pointer at the Core Features page (reference, "how this works"), matching the pattern the rest of the site's paired pages already follow correctly.
- **A "known limitation" that was actually describing a resolved gap** — `api-integrations/ai-assistants-mcp/known-limitations.md` had a bullet headed "No long-lived API token mechanism — resolved," which read as a stale leftover under a "Known limitations" heading; removed, since Personal Access Tokens are already documented on the linked Authenticating page and this bullet added nothing under a heading it contradicted.
- **`reference/glossary.md` undersold its own scope and was missing an entry** — its intro claimed to cover only terms "introduced across the Concepts section," when it in fact (correctly) also defines Module/Entitlement/Enablement (Modules section) and PAT/2FA (Core Features section); broadened the intro sentence to match, and added the missing **Tier A / Tier B / Tier C module** entry (defined and used repeatedly in `modules/overview.md` and `modules/third-party-and-federated-modules.md` but never in the glossary).
- **`reference/architecture-overview.md`'s "Non-functional considerations → Scalability" bullet restated its own "Deployment architecture" section's service list almost verbatim** — replaced with a cross-reference to that section instead of repeating the list.
- **Two placeholder-thin pages occupying full top-level sidebar slots** — `modules/roadmap.md` (two sentences of real content) and `api-integrations/extending-with-modules.md` (one paragraph, itself just a pointer to two other pages) each read as a disappointing dead end for a reader who clicked a real-looking nav entry. **Decided by: Agent**, revising Phase 6's and Phase 7's own "Decided by: Agent" calls to give each its own page — the content is genuinely too thin to justify a dedicated slot once actually written, which wasn't fully apparent until the page existed. Folded `roadmap.md`'s content into `modules/overview.md` as a new "Roadmap" section, and merged `extending-with-modules.md`'s content into `api-integrations/rest-api-overview.md` as a new "Module-contributed endpoints and tools" section (placed right where the base-URL/versioning section already gestured at modules). Removed both files, both `sidebars.ts` entries, and repointed the one inbound link each had (from `modules/overview.md`'s own "Where this fits" list, and from `rest-api-overview.md`'s base-URL section, which now links to itself instead of out).

**A factual bug found and fixed in passing, not part of the verbosity/coherence review itself, per this repo's "fix, don't defer" rule:** `README.md` (two places — the docs-pointer paragraph and the "Extensibility blueprint" bullet under Enterprise-ready, plus its "Known limitations" section) and `docs/solution-architecture.md` both stated that SCIM provisioning was "scoped as an implementable design... rather than built," identically to the still-genuinely-unbuilt separate-port provisioning API. `docs/enterprise-integration.md` (section header: "Built: SCIM provisioning") and the actual code (`backend/app/routers/scim.py`, `backend/tests/test_scim.py`) show SCIM has in fact been built — this website's own `api-integrations/scim-provisioning.md` (Phase 7) already documented it correctly as a working feature, which is what surfaced the contradiction. Root cause: SCIM shipped after the README/solution-architecture prose describing it as a future blueprint was written, and neither was updated at the time. Fixed all four call sites to correctly separate SCIM (built) from the separate-port provisioning API (still a blueprint) — `docs/decisions.md`'s own historical entries describing SCIM as not-yet-built were deliberately left untouched, since they accurately describe the state at the time each entry was written and this file is a historical log, not current-state documentation.

**Not changed, considered and judged acceptable:**
- The `api-integrations/ai-assistants-mcp/` client-setup split (5 pages, 2 under 20 lines each) — borderline fragmented, but each client genuinely has a different auth-config shape, and a reader who wants only their own client's steps benefits from not scanning past three others. Revisit if another client is added and the pattern starts to strain further.
- The homepage (`src/pages/index.tsx`) is a minimal hero (title, tagline, one "Read the docs" button) with no on-page pitch beyond the tagline — a deliberate, common Docusaurus pattern that routes a cold reader to `introduction/overview.md` in one click, which is where the real "what is this and why" answer already lives well. Not changed; flag if the user wants a richer landing page.
- Two "same point made twice" instances judged too minor to be worth the edit risk: `reference/architecture-overview.md`'s "Goals" vs. "Core principles" sections both mention security being built-in rather than bolted on (different framing, adjacent sections, not a verbatim repeat); a couple of "not glossed over / stated plainly" self-referential phrases in `modules/third-party-and-federated-modules.md` and `modules/building-your-own-module.md` (each appears once per page, not a systemic pattern).

**Verified:** `cd docs/website && npm run build` — clean, zero broken-link/broken-anchor errors (confirms both new cross-section anchors used above, `stages-and-baselining.md#renaming-and-deleting-stages` and `architecture-overview.md#deployment-architecture`, actually resolve, and that removing the two stub pages left no dangling reference); `npm run typecheck` — clean. `grep -rn` for both removed files' slugs (`roadmap`, `extending-with-modules`) across `docs/website/docs/` and `sidebars.ts` — no remaining reference. `grep -rn "SCIM"` across `README.md`, `docs/`, and `docs/website/docs/` for the same stale "blueprint/not built/deferred" phrasing — none remaining outside `docs/decisions.md`'s historical entries.

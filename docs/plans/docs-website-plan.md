...
**Last updated:** 2026-09-15 (Phase 3 complete).
...
| 6 | Modules section (own top-level nav) | [ ] Not started |
...
6. **Modules** (subpages — its own top-level entry, not nested under Core Features)
...
- *Building your own module* — the Tier A/B/C frontend integration model, module-contributed RBAC, module-contributed MCP tools, the "building a new module" checklist — adapted from `docs/modules.md`'s remaining sections. This is explicitly written for a third-party developer per that doc's own framing, so it belongs on the public site as-is (unlike the future-modules proposal doc, excluded per the "Open decisions" section above).

...
Five tests, using the fixed Gamma-3/Gamma-4 pair plus dynamically-created projects and (for the new fifth test) Gamma-1/Gamma-2: (1) create a sub-project via the create modal, confirm `MIRROR_ALL` requires — and a cancel correctly declines — a confirmation naming the parent, confirm "Child of:"/"Parent of:" labels and the tree view render the resulting pair, and confirm "Add sub-project" from a project's own admin page pre-fills the parent in the create modal; (2) the parent's admin page lists a child as a member source while the child's own admin page's own picker now offers other same-org projects too (the generalized mechanism, not evidence the authorization asymmetry weakened), effective members shows a forward-inherited user's provenance, and "Convert all inherited access to direct roles" converts it (reverted via a direct API call at the end of the test so the fixture stays idempotent across repeated runs — materializing is a one-way, permanent grant); (3) the no-org-role project manager persona creates a sub-project of a project they manage via the relaxed path, cannot detach it (`parent_required`) until granted `project_creator`, then can; (4) an org admin disables the relaxed-creation toggle, blocking the same persona's "Add sub-project", then restores it for later runs; (5) **new** — generalized member-source (any same-org project, not just a child, with `mirror_mode`) and project-referencing group members (`ProjectGroupMember.source_project_id`) both exercised end to end against unrelated Gamma-1/Gamma-2, self-healing via a `cleanup()` helper called both up front and in a `finally` block so a mid-test failure can't leave the shared fixture mutated for the next run — see docs/decisions.md's "Generalized cross-project group permissions" entry.
...
### 26. Project-wide file browser
**Persona:** Org Admin AlphaBeta. **Spec:** `project-files.spec.ts`.

`FileAsset` has no `project_id` of its own — it's only reachable via a join through whichever requirement/action/comment it's attached to — so `GET /projects/{id}/files` (`ProjectFilesPage.tsx`) combines all three origins into one list. The seed script's "Files on Alpha-1" step seeds one file via each origin specifically so this spec has a realistic multi-source project to assert against: a direct requirement attachment, a requirement action attachment, and a comment attachment. Confirms the overview's "Files" tile — which only ever counted direct attachments (`ProjectMetricsOut.file_count`'s narrower scope) — now links to this page instead of the unfiltered requirements list, confirms the nav rail's own "Files" entry works, and confirms a download link actually serves the file's bytes.

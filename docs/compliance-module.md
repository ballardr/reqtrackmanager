...
This document is the user/admin-facing reference for what the module does, how it's organised, and how to work with it day to day. For the module system's own architecture (how a "module" plugs into the backend/frontend/RBAC/MCP), see `docs/modules.md`. For the full, authoritative product requirements this module was built against, see `docs/plans/Compliance_Module_Requirements.md`; where this document and that one appear to disagree, the requirements document is the source of truth. For its implementation history, see `docs/plans/compliance-module-plan.md` and the "Compliance module plan" entries in `docs/decisions.md`.
...
**Standards Manager/Contributor** (a standard's own dedicated working group, for organisations that maintain standards internally) are narrower still: scoped to one specific standard, not every standard in the organisation. A Compliance Manager already satisfies either check with no per-standard grant needed — the same "a higher tier already retains full access" principle applied one level down. A standard's creator is automatically granted Standards Manager on it, and **a standard must always have at least one Standards Manager** — the last one can't be removed unless another explicit grant (direct or group) or the organisation's designated fallback compliance-managers group (Compliance's own org settings, `/standards/settings/:orgId/standardsManagement`) covers the floor, for organisations whose roles are managed via their identity provider rather than granted one person at a time. Manage this roster from a standard's own "Members" nav-rail section — either or both roles can be granted directly to a person, or to an org group as a whole (every current, and future, member of that group then holds the role), via the same search-as-you-type "Add member" control.
...
- Compliance reports/exports do not currently support the branded `ReportTemplate` styling (logo, accent colour, cover page) core requirement reports can use — they render with a fixed, unbranded layout. Not required by `docs/plans/Compliance_Module_Requirements.md` §29, and can be added later if wanted.
...
- Automated compliance rules, automatic compliance determination from required-action outcomes, compliance certificates/digital signatures, and customer-facing compliance portals are explicitly out of scope for this module today (`docs/plans/Compliance_Module_Requirements.md` §30) — the data model is deliberately built so none of them are precluded later.

...
6. **The MCP server (`mcp-server/`) is an additional retrieval path for Confidential requirement content, not an exception to isolation control #1** — it authenticates and authorizes every request exactly the same way the REST API does (by forwarding the caller's own token), so the isolation boundary above still holds for it. What's new is *where the data goes after retrieval*: if an organization connects a third-party AI platform to it, Confidential content now also flows to that platform's infrastructure. That's a data-flow/subprocessor question (see [vendor-and-subprocessor-management-policy.md](vendor-and-subprocessor-management-policy.md) item 6), not an isolation-control gap — the access-control boundary itself is unchanged.

...
7. **Non-interactive/programmatic access (the MCP server, `mcp-server/`) reuses the same session tokens rather than a separate credential type.** There is deliberately no shared service-account or API-key mechanism issuing its own broader access — every MCP tool call must present a caller's own access token, which the MCP server does nothing but forward to the backend, so this integration surface is subject to every control in this section (password/2FA/SSO issuance, the 12-hour default lifetime, `token_version` revocation) exactly as a browser session is. See [mcp-server.md](../../mcp-server.md)'s "Authentication model". `mcp-server` also serves its own `/login` convenience page, which briefly holds a submitted native password in memory only for the single relay call to the backend's existing login endpoint (never logged, never persisted, no new credential store) — functionally identical to a user calling the login endpoint themselves, just without hand-crafting the request.

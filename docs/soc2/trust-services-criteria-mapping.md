...
| CC6.2 | Registers and authorizes new internal/external users prior to granting access | Org admins provision members and assign roles; SSO users are provisioned on first login gated by an optional required-group check; server admin bootstrap is config-gated; public self-signup (opt-in, domain-gated in "org specified" mode) and admin-initiated project invites (org-opt-in `external_user_policy`) are two further, explicitly-authorized provisioning paths — see access-control-policy.md's Provisioning section and its Known Gap on unverified-email trust | `backend/app/routers/orgs.py`; `backend/app/services/oidc_provisioning.py` (`find_or_provision_user`, `meets_required_group`); `backend/app/routers/auth.py::signup`; `backend/app/services/invites.py` | ✅ |
...
| CC6.6 | Implements controls to protect against threats from outside system boundaries | TLS termination and network exposure are the deployer's responsibility (the app serves plain HTTP internally by design); CORS origin allow-list; SSRF guard on org-configured OIDC endpoints; per-account lockout on the 2FA challenge step after repeated failed codes | [deployment.md](../deployment.md) §TLS and reverse proxy; `backend/app/main.py` (`CORSMiddleware`); `backend/app/services/oidc_client.py` (`_assert_safe_external_url`); `backend/app/routers/auth.py` (`verify_2fa`) | 🟡 No rate limiting or brute-force/account-lockout protection on the first-factor *password* check exists yet; the second-factor (2FA) step is now locked out after repeated failures — see [policies/access-control-policy.md](policies/access-control-policy.md) Known Gaps |
...
| CC6.8 | Prevents or detects and acts upon unauthorized/malicious software | Endpoint/host-level malware protection, dependency vulnerability scanning | No automated dependency/container scanning exists in this repository | 🔴 Gap — see [policies/change-management-and-secure-development-policy.md](policies/change-management-and-secure-development-policy.md). **Note (module system Phase 1, plans/compliance-module-plan.md):** the modular feature system's third-party discovery sources (Python entry points, `EXTRA_MODULES_PATH`) are gated behind `ALLOW_EXTERNAL_MODULES`, default `false` — a deliberate, documented mitigation layered on top of this still-open gap (a plugin-loading mechanism that can auto-import and run third-party code materially raises the stakes of having no automated scanning), not a closure of it; see [policies/vendor-and-subprocessor-management-policy.md](policies/vendor-and-subprocessor-management-policy.md) point 7 and its Known Gaps entry. **Note (module system follow-up, 2026-09-07 — Tier C / Module Federation):** a Tier C frontend module raises these same stakes further still, not merely in the same way as point 7's backend module — it runs with **no sandbox at all** (same origin/DOM/cookies/component tree as the host), where a backend module is at least bounded by the backend process's own authorization checks and Tier B (the modular feature system's *other* dynamic frontend option) is bounded by an iframe sandbox. Gated by the same `ALLOW_EXTERNAL_MODULES` flag, not a second one; see [policies/vendor-and-subprocessor-management-policy.md](policies/vendor-and-subprocessor-management-policy.md) point 8 and its Known Gaps entry 5. |
...
| C1.2 | Disposes of confidential information to meet the entity's objectives | Account deactivation; organization disable/hard-delete; retention/disposal policy | [policies/data-retention-and-disposal-policy.md](policies/data-retention-and-disposal-policy.md); `backend/app/services/org_deletion.py` | 🟡 Genuine hard deletion now exists for organizations (`DELETE /orgs/{id}`, cascading through the database and object storage — see `docs/decisions.md`'s "Organisation disable and hard delete" section); no equivalent exists yet for an individual user's or project's data within a still-active organization, and no documented retention schedule exists — Company action required for the remainder
...
2. **No account lockout / brute-force protection on the first-factor password check.** (CC6.6) The 2FA challenge step now has per-account lockout after repeated failed codes; the password check itself remains unprotected — a large-enough keyspace that this is judged lower urgency than the (now-closed) 2FA gap.
